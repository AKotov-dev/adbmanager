...
begin
  ...
  try
    ...
    begin
      ...
      for i := 0 to AppListBox.Count - 1 do
        if AppListBox.Checked[i] = True then
          adbcmd := adbcmd + 'adb shell "pm uninstall --user 0 ' + AppListBox.Items[i] +
            ' 2> /dev/null ; pm uninstall ' + AppListBox.Items[i] + ' 2> /dev/null";';
...
begin
  ...
  for i := 0 to AppListBox.Items.Count - 1 do
  begin
    if AppListBox.Selected[i] then
    begin
      if s <> '' then
        s := s + sLineBreak;
      s := s + AppListBox.Items[i];
    end;
  end;
